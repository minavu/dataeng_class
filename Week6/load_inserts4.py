...
def getSQLcmnds(rowlist):
	...
	for row in rowlist:
		valstr = row2vals(row)
		cmd = f"INSERT INTO staging_{TableName} VALUES ({valstr});"
		cmdlist.append(cmd)
	return cmdlist
...
# create the target table and staging table
# assumes that conn is a valid, open connection to a Postgres database
def createTable(conn):

    for x in [f"{TableName}", f"staging_{TableName}"]:
...
def load(conn, icmdlist):
	with conn.cursor() as cursor:
		...
		cursor.execute(f"INSERT INTO {TableName} SELECT * FROM staging_{TableName};")
		...
		cursor.execute(f"DROP TABLE staging_{TableName} CASCADE;")
		conn.commit()
