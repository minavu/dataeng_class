#program to load Census ACS data using INSERT statements
#run with -h option to see the command line options
...
DataFile = "no file yet"
CreateDB = False #to indicate whether DB table should be (re)created

def row2vals(row):
    for key in row:
        if not row[key]:
            row[key] = 0 #to handle the null values
        row['County'] = row['County'].replace("'", "") #to eliminate quotes within literals
...
#to read the input data file into a list of row strings
def readdata(fname):
...
#to convert list of data rows into list of SQL 'INSERT INTO ...' commands
...
#assumes that conn is a valid and open connection to a Postgres database
